...
class GhepVoiRawThat(unittest.TestCase):
    ...
    def test_so_khop_raw_that(self):
        ...
        sample = next((p for p in (folder / f"{h}.json.gz" for h in
                       ...
                       if p.exists()), None)
        if sample is None and folder.exists():
            sample = next(folder.glob("*.json.gz"), None)
        if sample is None:
            self.skipTest("không có file raw mẫu nào")
        dump = json.loads(gzip.decompress(sample.read_bytes()))
        that = next((r["response"] for r in dump.get("responses") or []
                     if r.get("url") == cf.DETAIL_BLOCK_URL), None)
        if that is None:
